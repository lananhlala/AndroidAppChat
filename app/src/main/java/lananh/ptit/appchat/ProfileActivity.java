            ...
            public void onClick(View v) {
                ...
                if(mCurrent_state.equals("friend")){
                    mFriendDatabase.child(mCurrentUser.getUid()).child(user_id).removeValue().addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
                        public void onSuccess(Void aVoid) {
                            // xoa ca bang notification
                            mFriendDatabase.child(user_id).child(mCurrentUser.getUid()).removeValue().addOnSuccessListener(new OnSuccessListener<Void>() {
                                @Override
                                public void onSuccess(Void aVoid) {
                                    mNotificationDatabase.child(mCurrentUser.getUid()).removeValue().addOnSuccessListener(new OnSuccessListener<Void>() {
                                        @Override
                                        public void onSuccess(Void aVoid) {
                                            mCurrent_state = "not_friend";
                                            btMakeFriend.setVisibility(View.VISIBLE);
                                            btMakeFriend.setEnabled(true);
                                            btMakeFriend.setText("Kết bạn");
                                            btDeclineFriend.setVisibility(View.INVISIBLE);
